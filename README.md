# 文献下载器（网页版 · liter-loader）

把 `literature-downloader` Skill 的「批量采集模式」做成一个本机网页应用：

**关键词检索 → 页面看清单 → 勾选 → 一键下载 → 三份汇总文件 + 全文落到 `paperdown/`。**

只检索 `config.json` 期刊白名单内的文献；只下载合法可直接访问的开放获取（OpenAlex / Europe PMC / Unpaywall）或出版社直连全文，**不绕过付费墙、不使用盗版来源、不共享账号**。
...
| 5. 检索前用 LLM 生成新关键词 | 点「AI 扩展关键词」（或勾选「每次检索自动扩展」）：把每个概念块、检索范围与期刊白名单领域一起交给大模型，为**每个概念块生成 8-10 个**相关新词；扩展结果先以可勾选标签列出，确认后才与用户关键词一起检索。详见「三之三、AI 关键词扩展」。 |
| 6. 可配置 LLM 模型与 key | 两种方式：① 网页右上角 **「LLM配置」按钮** 打开弹窗直接填 key（写回 `config.json`，写前校验 + 自动备份，界面不回显明文）；② 手工编辑 `config.json` 的 `llm` 段（`providers`/`models`/`api_key`/`api_key_env`）。详见「三之四、网页上的配置弹窗」。 |
| 7. 新增 Scopus 与 Web of Science 渠道 | 结果页渠道勾选区新增两项；两者都需要**机构订阅的 API key**（Scopus 用 Elsevier key、WoS 用 Clarivate key），在「LLM配置」弹窗里填入即可启用；未配置时会明确提示并跳过，不影响其他渠道。详见「三之五、Scopus 与 Web of Science」。 |
...
- `journals` 就是「文献清单」：只有期刊命中清单的检索结果才会出现、才会被下载。
...
- 白名单为空时，检索会直接返回 400 并提示先编辑 `config.json`，避免出现「静默 0 条」的困惑。
- 写坏 JSON 会返回明确的中文报错（含行列位置），不会静默失败。
...
Crossref 的 API 不支持字段限定，只能按相关度返回（内部是词袋 OR）。因此：
...
- **优先级铁律：字面 `api_key` 永远优先于环境变量。** 写在 `api_key_env` 里的变量若未设置，**不会**把已填写的字面 key 清空（早期版本正是在这里清空了密钥，导致「弹窗保存 key 成功，紧接着测试连接却报没有 api_key」）。这种情况只会在配置警告里提示一句，不影响已填的 key；界面上模型的 key 来源也会显示为「config.json（api_key）」或「环境变量 XXX」。
...
- **不用 DeepSeek 也可以**：任何 OpenAI 兼容端点（自建 vLLM/Ollama、其它厂商）只要在 `providers` 里设 `type: "openai-compatible"` + `base_url`，并在 `models` 里指向它即可。`provider_presets` 会同步给前端用于提示。
- 改完 `config.json` 在网页上点「重载配置」，然后点 **「测试模型连接」** 验证 key/base_url/模型名；成功会显示端点与模型回复预览，失败会给出具体原因与排查清单（缺少 api_key、HTTP 401、超时、域名不可达等）。
- 弹窗里的 **「测试该模型连接」会先自动保存输入框里刚填的 key，再发起测试**，结果直接显示在弹窗内，**不会关闭弹窗**（关不关由你决定：✕ / 完成并关闭 / Esc）。测试超时上限 60 秒，避免「点了没反应」。
...
## 三之四、网页上的配置弹窗（LLM配置 按钮）

右上角 **「LLM配置」** 打开弹窗，无需手工改 JSON 就能填 key：

| 区块 | 能改什么 | 说明 |
| --- | --- | --- |
| 1. 大模型 | 选择模型 → 填/换该模型的 API key；「测试该模型连接」 | 保存后立即重载，`/api/llm/models` 里的 `has_api_key` 会变为 true |
| 2. 检索渠道密钥 | **Scopus / Web of Science（均为可选）** | 每个渠道有「保存 key」「保存并启用」「停用该渠道」三个按钮；不填 key 也能正常使用其它四个免费渠道 |
| 3. 自建 / 其它厂商端点 | 提供商名称、`base_url`、API key | 保存时会在 `llm.providers` 里新增/更新该提供商，之后可在 `llm.models` 里引用 |

关闭方式有三种：右上角 **✕ 关闭**、底部 **完成并关闭**、按 **Esc**；点弹窗外面的灰色区域也会关闭。保存结果会同时出现在弹窗顶部提示区与右下角的浮层提示（toast），不会「点了看不出反应」。

安全与可靠性：

- **不回显明文**：已完成保存的密钥只显示「已配置 / 未配置」，输入框永远为空；留空 = 不改动，填 `-` = 清空该项。
- **写前校验 + 自动备份**：先写入临时文件并调用配置解析器校验，通过后才用原子替换覆盖 `config.json`，同时保留 `config.json.bak`；校验失败会**自动回滚**原文件（已有单测覆盖）。
- **只改点名项**：白名单、年份、其它渠道开关等未提交的字段原样保留；「保存并启用」只额外写一个 `sources.<渠道>=true`。
- 密钥只写进本机 `config.json`（已在 `.gitignore` 中忽略），不会出现在 `/api/config`、运行快照或三份交付文件里。
- 接口：`POST /api/config/edit`，入参形如
  `{"model_keys": {"deepseek-flash": "sk-..."}}`、`{"source_keys": {"scopus": "..."}, "sources": {"scopus": true}}`、
  `{"provider": {"name": "my-endpoint", "base_url": "https://.../v1", "api_key": "..."}}`，
  返回 `{changed, backup, config}`（不含密钥内容）。

> 排查记录：早期版本有一个会让弹窗「盖住整页、点什么都没反应」的 bug——CSS 里 `.modal { display: flex }` 覆盖了浏览器对 `[hidden]` 的默认 `display:none`，于是弹窗虽然设了 `hidden` 依然可见并吃掉全部点击。现在 `style.css` 用 `[hidden] { display: none !important; }` 统一钉死，并加了 `scripts/mock_dom_check.mjs` 这类回归检查（会断言页面上存在该规则、并模拟点击关闭/保存）。
>
> 另一个已修复的关键 bug：`api_key_env` 指向的环境变量**未设置**时，会把配置里已填的 `api_key` 覆盖成空字符串，于是「在弹窗里保存 key → 立刻测试连接」永远报「没有配置 api_key」。现在字面 key 优先，`api_key_env` 只作为回退，`tests/test_key_precedence.py` 专门守住这条规则。

---

## 三之五、Scopus 与 Web of Science（可选渠道）
...
1. **① 检索配置**：显示当前白名单期刊（含 ISSN 与指标）、年份范围、数据源状态，以及 AI 关键词扩展的模型与 key 状态。改完 `config.json` 点右上角「重载配置」；点「LLM配置」可以直接填各种 key（见「三之四」）。
2. **② 关键词检索**：填关键词，例如
   ...
   可选：检索范围（仅标题 / 标题+摘要 / +关键词 / 全部字段）、起止年份、每个数据源最多条数、数据源开关、「只在有开放获取信号的文献中检索」，以及 AI 关键词扩展（见「三之三」）。
...
- `文章地址总表.csv`：列＝`序号, 题名, 年份, 期刊或来源, 影响因子, JCR分区, 指标年份, 指标来源, 索引情况, DOI, 文章地址, 是否勾选, 是否下载成功, 全文状态, 文件或原因, 下一步建议`（UTF-8-SIG，Excel 双击不乱码）。
...
状态一律用中文：`已下载 PDF`、`已保存网页全文`、`已保存 XML 全文`、`只找到题录`、`平台限制直连下载`、`链接失效`、`被限流`、`已排除`、`尝试失败`。
...
**没下成通常不是程序坏了**，而是 ACS、Elsevier、SpringerLink 等平台限制非浏览器/非机构网络直连。遇到这种情况请用 DOI 走学校图书馆、机构 VPN、馆际互借、机构仓储或直接联系作者。程序绝不会把失败包装成成功。

> 本机网络实测：`europepmc.org`、`pmc.ncbi.nlm.nih.gov`、`ncbi.nlm.nih.gov` 的网页域会被拦截（403），所以这些域名的链接会被主动跳过（不浪费尝试次数），PMC 全文改走 Europe PMC 的 REST 域（可正常访问）。
...
`config.json` 里会存 LLM / Scopus / Web of Science 的 api_key，因此它**默认不提交**；仓库只保留 `config.example.json` 作为模板。做法：

1. **`.gitignore` 用「先全忽略再放回示例」的写法**，以后无论配置文件叫什么名字都不会误提交：
   ...
   *.bak
   ...
   同时覆盖备份（`config.json.bak`）、凭据类文件（`credentials.json`、`secrets.*`、`token.txt`）、运行产物（`paperdown/`、`literature_runs/`）、本地临时目录与编辑器/系统文件。

2. **提交前跑一次扫描**（不依赖网络、不调用 git 子进程，自己解析 `.gitignore`）：

   ```powershell
   python scripts\check_secrets.py          # 有问题时非零退出
   python scripts\check_secrets.py --json   # 便于接 CI
   ```

   它会分别报告「会被提交的文件」与「已被忽略的文件」里的命中，并反向确认 `config.json`、`config.json.bak` 处于已忽略状态。判定时会跳过示例/测试用的占位密钥（`sk-test-secret`、`sk-...`、`env:XXX` 等），但**不会**放过真实样式的密钥——包括 `sk-abcdef1234…` 这种看起来像占位、实际可能就是真 key 的字符串。

3. **装个钩子，彻底避免手滑**：

   ```powershell
   python scripts\install_git_hooks.py     # 写入 .git/hooks/pre-commit
   ```

   之后每次 `git commit` 都会先扫描，命中就中止提交（确认无碍可 `git commit --no-verify` 跳过）。钩子只在本机 `.git/hooks` 里，不会上传。

4. **两个提醒**：
   - `.gitignore` 只对「未被跟踪的文件」生效。如果某个含密钥的文件已经被 `git add` 过，需要先 `git rm --cached <file>` 再提交，并且**该密钥应视为已泄露，建议在服务商后台轮换**。
   - `git add -f` 会强行绕过忽略规则，别用它加配置或产物文件。
...
设计上刻意复用 Skill 的既有约定：候选表字段名、状态标签、中文措辞、期刊指标合并进每一行、`文章地址总表.csv` 的列、合规说明。Skill 目录本身**不做任何修改**。

---

## 十、常见问题

- **检索结果很少**：白名单太窄。看报告里「白名单外被过滤的期刊」小节，把要的期刊加进 `journals` 再检索。
- **中文关键词几乎没结果**：OpenAlex/Crossref/PubMed 以英文为主，建议中英并用或补英文同义词；界面会给出提示。
- **全是「平台限制直连下载」**：多数是订阅期刊的常见情况，属正常结果，按「下一步建议」走学校库/馆际互借/联系作者。
- **Scopus / Web of Science 没结果**：这是**可选渠道**，先看是不是没配 key（勾选框旁有「需 key（可选）」标记，提示区会写 `missing_api_key`）。配了 key 还报 401/`AUTHENTICATION_ERROR`，说明 key 无效或所在机构没有该库的订阅权限——这种情况请联系学校图书馆确认是否有机构 API 权限。
- **想改 key 但不想手改 JSON**：点右上角「LLM配置」，填完保存即可（会自动备份 `config.json.bak`；留空=不改，填 `-`=清空）。弹窗可用 ✕ / 完成并关闭 / Esc 关闭。
- **想换输出位置**：改 `config.json` 的 `download.output_root`（支持绝对路径）。
- **想彻底关掉某数据源**：`search.sources` 里对应项设为 `false`，或在界面上取消勾选。
- **端口不是 8765**：说明默认端口被占用，用终端打印的实际地址即可（`--port` 可指定起始端口）。
